	...
	class Editor : IGameItem
	{
		...
		public void Draw(GameTime gameTime)
		{
			...
			if (CurState == State.Main)
			{
				...
				Map.DrawLight(Game.GraphicsDevice, Offset);
				...
				Game.SpriteBatch.DrawString(Font, $"(B)locks: {DrawBlocks}   De(c)als: {DrawDecals}   (P)lacement: {CurPlacementMode}   (L)ight Base: {Map.LightBase}   L(i)ght: {LightColor}", new Vector2(80, 12), Color.White);
				Game.SpriteBatch.End();
			}
		...
		public void Update(GameTime gameTime)
		{
			...
			if (CurState == State.Main)
			{
				...
				else
				{
					...
					else if (CurPlacementMode == PlacementMode.Light)
					{
						if (Game.Input.Mouse.LeftButton == InputState.ButtonState.JustPressed)
						{
							Map.Lights.Add(new Light
							{
								Rectangle = new Rectangle(mX - 64, mY - 64, 128, 128),
								Color = LightColor,
							});
						}
						if (Game.Input.Mouse.RightButton == InputState.ButtonState.JustPressed)
						{
							var light = Map.Lights.FirstOrDefault(l => l.Rectangle.Contains(mX, mY));
							if (light != null)
							{
								Map.Lights.Remove(light);
							}
						}
						if (Game.Input.Mouse.ScrollWheel != 0)
						{
							int chg = Game.Input.Mouse.ScrollWheel / 24;
							foreach (var l in Map.Lights.Where(l => l.Rectangle.Contains(mX, mY)))
							{
								bool x = Game.Input.Keyboard.IsDown(Keys.X);
								bool y = Game.Input.Keyboard.IsDown(Keys.Y);
								bool both = x == y;

								if ((x || both) && ((l.Rectangle.Width + 2 * chg) > 0))
								{
									l.Rectangle.X -= chg;
									l.Rectangle.Width += 2 * chg;
								}
								if ((y || both) && ((l.Rectangle.Height + 2 * chg) > 0))
								{
									l.Rectangle.Y -= chg;
									l.Rectangle.Height += 2 * chg;
								}
							}
						}
					}
				...
				{
					switch (CurPlacementMode)
					{
						case PlacementMode.Tile:
							CurPlacementMode = PlacementMode.Decal;
							break;
						case PlacementMode.Decal:
							CurPlacementMode = PlacementMode.Light;
							break;
						case PlacementMode.Light:
							CurPlacementMode = PlacementMode.Tile;
							break;
					}
				}
				...
				if (Game.Input.Keyboard.IsDown(Keys.LeftControl) && Game.Input.Keyboard[Keys.S] == InputState.ButtonState.JustPressed)
				{
					...
					if (save.ShowDialog() == System.Windows.Forms.DialogResult.OK)
					{
						...
						{
							IntermediateSerializer.Serialize(writer, Map.ToMapData(), null);
						}
					}
				}
				if (Game.Input.Keyboard.IsDown(Keys.LeftControl) && Game.Input.Keyboard[Keys.L] == InputState.ButtonState.JustPressed)
				{
					...
					if (open.ShowDialog() == System.Windows.Forms.DialogResult.OK)
					{
						...
						{
							var lvl = IntermediateSerializer.Deserialize<MapData>(reader, null);
							Map = new Map(Game, lvl);
						}
					}
				}
				else if (Game.Input.Keyboard[Keys.L] == InputState.ButtonState.JustPressed)
				{
					var color = new System.Windows.Forms.ColorDialog();
					color.Color = System.Drawing.Color.FromArgb(Map.LightBase.A, Map.LightBase.R, Map.LightBase.G, Map.LightBase.B);
					if (color.ShowDialog() == System.Windows.Forms.DialogResult.OK)
					{
						Map.LightBase = new Color(color.Color.R, color.Color.G, color.Color.B, color.Color.A);
					}
				}
				if (Game.Input.Keyboard[Keys.I] == InputState.ButtonState.JustPressed)
				{
					var color = new System.Windows.Forms.ColorDialog();
					color.Color = System.Drawing.Color.FromArgb(LightColor.A, LightColor.R, LightColor.G, LightColor.B);
					if (color.ShowDialog() == System.Windows.Forms.DialogResult.OK)
					{
						LightColor = new Color(color.Color.R, color.Color.G, color.Color.B, color.Color.A);
					}
				}
			}
			else if (CurState == State.SelectTile)
